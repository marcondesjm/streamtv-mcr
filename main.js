...
const buildVideoArgs = (filePath, offsetSeconds, fullRtmpUrl, vfFilter) => {
  const args = [
    ...
    '-filter_complex', `[0:a][1:a]amix=inputs=2:duration=first[aout]${vfFilter ? `;[0:v]${vfFilter}[vout]` : ''}`,
...
const buildRadioArgs = (radioUrl, bannerUrl, fullRtmpUrl, vfFilter) => {
  ...
  // Determina se o banner é válido
  ...
  // Input 0: Banner de vídeo (imagem ou fundo preto)
  if (hasBannerFile) {
    // Arquivo local de imagem: usa image2 com loop
    args.push('-f', 'image2', '-loop', '1', '-r', '1', '-i', resolvedBanner);
  } else if (hasBannerUrl) {
    // URL de imagem: baixa e usa como static frame (sem loop flag)
    args.push('-r', '1', '-i', resolvedBanner);
  } else {
    // Sem banner: usa fundo preto gerado pelo lavfi (infinito por natureza, sem flags de loop)
  ...
  // Output: encode e envio RTMP
  // Mapeia: vídeo do input 0, áudio do input 1
  const videoFilter = vfFilter ? `[0:v]${vfFilter}[vout]` : null;

  if (videoFilter) {
    args.push('-filter_complex', videoFilter, '-map', '[vout]');
  } else {
    args.push('-map', '0:v');
  }
...
ipcMain.handle('start-stream', async (_event, { videoPath, offsetSeconds, rtmpUrl, streamKey, mode, overlayConfig, programTitle, fallbackUrl, fallbackBanner }) => {
  ...
  if (isWebRadio || (!videoPath && fallbackUrl)) {
    ...
    args = buildRadioArgs(radioSource, fallbackBanner, fullRtmpUrl, vfFilter);
  } else if (videoPath === '__camera__' || mode === 'screensaver' || !videoPath) {
    args = buildScreensaverArgs(fullRtmpUrl, vfFilter);
  } else {
    args = buildVideoArgs(resolveFilePath(videoPath), offsetSeconds, fullRtmpUrl, vfFilter);
  }
...
ipcMain.handle('switch-stream', async (_event, { videoPath, offsetSeconds, overlayConfig, programTitle, fallbackUrl, fallbackBanner, mode }) => {
  ...
  if (isWebRadio || (!videoPath && fallbackUrl)) {
    const radioSource = (videoPath === '__webradio__' && fallbackUrl) ? fallbackUrl : (fallbackUrl || videoPath);
    args = buildRadioArgs(radioSource, fallbackBanner, currentRtmpUrl, vfFilter);
  } else if (videoPath === '__camera__' || !videoPath) {
    args = buildScreensaverArgs(currentRtmpUrl, vfFilter);
  } else {
    args = buildVideoArgs(resolveFilePath(videoPath), offsetSeconds, currentRtmpUrl, vfFilter);
  }
