...
const WebRadioView: React.FC<{ radioUrl?: string; bannerUrl?: string }> = ({ radioUrl, bannerUrl }) => {
  return (
    <div style={{ width: '100%', height: '100%', position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: '#050505', backgroundImage: `url(` + "$" + `{bannerUrl})`, backgroundSize: 'cover', backgroundPosition: 'center', backgroundRepeat: 'no-repeat' }}>
      {bannerUrl ? (
        <div style={{ position: 'absolute', inset: 0, backgroundColor: 'rgba(0,0,0,0.6)' }} />
      ) : (
        <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '60px' }}>📻 RADIO WEB</div>
      )}
      {bannerUrl && <img src={bannerUrl} alt="Radio Banner" style={{ maxWidth: '80%', maxHeight: '80%', objectFit: 'contain', zIndex: 5 }} />}
      {radioUrl && (
        <audio src={radioUrl} autoPlay loop controls style={{ position: 'absolute', bottom: '20px', zIndex: 10, width: '80%', opacity: 0.9 }} />
      )}
    </div>
  );
};
...
function App() {
  ...
  const [streamKey, setStreamKey] = useState('live_1492739470_skWF8fBFUcNr0h5Olhw7yljHElvfGK');
  ...
  useEffect(() => {
    if (activeTab === 'schedule' && timelineRef.current) {
      ...
      if (viewDate === todayStr) {
        ...
        const scrollPos = (minutes / 60) * 200 - 300;
...
$content3 = @"
        {activeTab === 'mcr' && (
          <MCRPro videos={videos.map(v => ({ id: v.id, title: v.title, duration: v.duration || '00:05:00' }))} />
        )}

        {activeTab === 'live' && (
          <div style={{ flex: 1, backgroundColor: '#06060a', display: 'flex', flexDirection: 'column', overflow: 'hidden', padding: '10px', gap: '10px', fontFamily: 'monospace' }}>
                  ...
                  <button onClick={async () => {
                      ...
                      try {
                        ...
                        const result = await ipcRenderer.invoke('start-stream', {
                          ...
                          rtmpUrl, streamKey, mode: isLive ? 'video' : 'radio', overlayConfig,
                          programTitle: liveProgram?.video?.title || '',
                          fallbackUrl: fallbackRadioUrl,
                          fallbackBanner: fallbackBannerUrl
                        });
            ...
            <div style={{ flex: 1, display: 'grid', gridTemplateColumns: '1fr 380px', gap: '10px', overflow: 'hidden' }}>
                    ...
                    <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>
                      {isLive && liveProgram ? (
                        liveProgram.video.platform === 'camera' ? (
                          ...
                          <WebRadioView key={`radio-` + "$" + `{liveProgram.id}`} radioUrl={liveProgram.video.radioUrl} bannerUrl={liveProgram.video.bannerUrl} />
                        ) : (
                          <video key={`video-` + "$" + `{liveProgram.id}`} ref={videoRef} autoPlay muted loop style={{ width: '100%', height: '100%', objectFit: 'contain' }} src={liveProgram.video.id} />
                        )
                      ) : fallbackRadioUrl ? (
                ...
                <div style={{ height: '180px', display: 'grid', gridTemplateColumns: '1fr 200px', gap: '10px' }}>
                  ...
                  <div style={{ backgroundColor: '#1a1a24', border: '1px solid #2a2a35', padding: '15px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
                    ...
                    <div style={{ height: '4px', backgroundColor: '#000', borderRadius: '2px', overflow: 'hidden' }}>
                      <div style={{ height: '100%', backgroundColor: '#646cff', width: isLive ? ` + "$" + `{(offsetSeconds / (liveProgram?.durationMinutes * 60 || 1)) * 100}%` : '0%' }} />
                    </div>
                  </div>
              ...
              <div style={{ backgroundColor: '#1a1a24', border: '1px solid #2a2a35', display: 'flex', flexDirection: 'column', overflow: 'hidden', borderRadius: '4px' }}>
                <div style={{ padding: '12px', backgroundColor: '#14141d', borderBottom: '1px solid #2a2a35', display: 'flex', justifyContent: 'space-between' }}>
                ...
                <div style={{ padding: '15px', backgroundColor: '#000', borderTop: '1px solid #2a2a35' }}>
                   ...
                   <div style={{ fontSize: '12px', color: '#fff' }}>SYSTEM_NOMINAL_100%</div>
                </div>
              </div>
...
$finalContent | Set-Content 'c:\Users\Marcondes\Desktop\StreamTV-main\src\App.tsx' -Encoding UTF8
